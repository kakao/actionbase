    ...
    StringSpec(
        {
            ...
            "a query a name still points at cannot be dropped" {
                ...
                queries.list().test().assertNext { it shouldBe emptyList() }.verifyComplete()
            }
            ...
            "listing answers with what the status asks for" {
                ...
                queries.list().test().assertNext { it.map { query -> query.id } shouldBe listOf(registered.id) }.verifyComplete()
                queries.list(MetadataStatus.INACTIVE).test().assertNext { it shouldBe emptyList() }.verifyComplete()
                queries.aliases().test().assertNext { it shouldBe emptyList() }.verifyComplete()
            }
