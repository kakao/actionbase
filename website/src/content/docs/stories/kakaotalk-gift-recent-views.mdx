---
title: KakaoTalk Gift - Recent Views
...
When a view event arrives, Actionbase writes it to WAL with `queue=true` and returns immediately. No locking, no state computation—just append to the log.

### Stage 2: Async Processing

A Spark Streaming job consumes the WAL entries and sends mutations back to Actionbase. The processor:
...
Actionbase processes the mutations in the background—acquiring locks, updating state, computing indexes and counts. Data is typically reflected within tens of milliseconds.
...
This pattern works well for high-frequency, low-criticality interactions where slight delay is acceptable.
