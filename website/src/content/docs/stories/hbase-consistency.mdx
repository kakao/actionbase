...
Deployed. Migration verified. But data inconsistencies can still occur during operations.
...
HBase mutations execute State, Index, and Count together in a single batch operation. But if a region server fails mid-batch or network issues cause partial writes, some may update while others don't. When consistency between State, Index, and Count breaks, queries return incorrect results.
