...
One table can have many contracts. One service needs 100 items at once, another paginates 10 at a time. One requires strong consistency, another is fine with eventual. Each usage pattern is a separate contract, protected independently.
...
| Aspect             | Traditional (Pact-style)     | Actionbase                           |
| ------------------ | ---------------------------- | ------------------------------------ |
...
The contract testing framework was a blocking issue for open-sourcing Actionbase—it contained too much internal information. We decided to remove it for now. Our plan is to incrementally open-source this framework, sanitizing internal details as we go. We're also exploring a new model—one where anyone can contribute contracts. What that looks like is still evolving. In the meantime, we'll share what we can through talks and presentations. See the [Roadmap](/community/roadmap/) for updates.
