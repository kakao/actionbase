...
Services rely on specific behaviors: pagination size, index filters, query direction, batch semantics, consistency guarantees. Combine these, and the number of possible usage patterns explodes.
...
The contracts we write cover the usage patterns that services depend on. Writing these contracts takes more work, but it's a choice we made—for trust. As we open-source Actionbase, we're exploring a new model—one where anyone can contribute contracts. What that looks like is still evolving.
...
- **Contract tests** — verify promises to each service (co-owned)
...
Contracts evolve with service requirements. Each update increments the version.
...
| **Evolution**      | Version negotiation          | Cumulative, versioned                |
...
The contract testing framework was a blocking issue for open-sourcing Actionbase—it contained too much internal information. We decided to remove it for now. Our plan is to incrementally open-source this framework, sanitizing internal details as we go. In the meantime, we'll share what we can through talks and presentations. See the [Roadmap](/community/roadmap/) for updates.
