...
1. Integration: {
  ...
  Write Test: {
    Decision: New Table? {shape: diamond}
    ...
    Decision -> Table Test: Yes
    Decision -> Composition Test: No
  }

  Deploy: Website Deploy
  Review: Requirements OK? {shape: diamond}
  Locked: Contract Locked

  Request -> Write Test
  Write Test -> Deploy
  Deploy -> Review
  Review -> Write Test: No
  Review -> Locked: Yes
}
...
2. Protection: {
  Change: Code Change
  PR: PR
  Contracts: All Contracts {shape: diamond}
  Merge: Merge
  Block: Blocked

  Change -> PR
  PR -> Contracts
  Contracts -> Merge: Pass
  Contracts -> Block: Fail
}

1. Integration.Locked -> 2. Protection.Contracts: adds to
```
