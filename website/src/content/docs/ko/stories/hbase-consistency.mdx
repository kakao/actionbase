...
배포했습니다. 마이그레이션도 검증했습니다. 하지만 운영 중에도 데이터 불일치가 발생할 수 있습니다.
...
HBase 뮤테이션은 State, Index, Count를 한 번의 배치 오퍼레이션으로 실행합니다. 하지만 리전 서버가 배치 중간에 실패하거나 네트워크 문제로 부분 쓰기가 발생하면, 일부만 갱신될 수 있습니다. State, Index, Count 간의 정합성이 깨지면 쿼리가 부정확한 결과를 반환합니다.
