...
Actionbase 운영 중 발생할 수 있는 데이터 불일치를 감지하고 보정하는 방법입니다.
...
HBase 뮤테이션은 State, Index, Count를 한 번의 배치 오퍼레이션으로 실행합니다. 하지만 리전 서버가 배치 중간에 실패하면? 또는 네트워크 문제로 부분적인 쓰기가 발생하면?

결과: 일부는 갱신되고 일부는 갱신되지 않을 수 있습니다. State, Index, Count 간의 정합성이 깨지면 쿼리가 부정확한 결과를 반환합니다.
...
불일치가 감지되면 서비스 SLA 요구사항에 따라 주기적으로 보정합니다.

State가 정답입니다. Index와 Count는 State로부터 재생성할 수 있습니다.
