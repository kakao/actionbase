...
프로덕션 Actionbase 앞단의 Nginx Ingress가 모든 요청과 응답을 Access Log로 기록합니다. 이 로그를 미러링해서 새 버전에 재생합니다. 로그 기반이라 실제 서비스 트래픽에는 영향이 없습니다.
...
1. **Capture**: Kubernetes Nginx Ingress가 모든 요청과 응답을 Kafka로 기록
2. **Mirror**: 새 버전이 실행 중인 테스트 환경에 미러링
3. **Compare**: 프로덕션 응답과 비교
4. **Verify**: 일치하면 배포, 불일치하면 차단
