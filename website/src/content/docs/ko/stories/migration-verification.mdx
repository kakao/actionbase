...
검증 시점에 경계 데이터가 존재합니다. Source DB Dump와 CDC Snapshot의 시점이 정확히 일치하지 않기 때문입니다.

경계 데이터는 현재 검증 윈도우에서 제외합니다. 검증 윈도우가 `T-1`일 00:00 ~ 23:59라면, 경계에 해당하는 데이터는 다음 윈도우 `T`일 검증에 포함됩니다. 윈도우가 슬라이딩하면서 모든 데이터가 검증됩니다.
