...
<Aside type="caution">
  인증이 적용되지 않습니다. [API 레퍼런스](/ko/api-references/#authentication)를 참고하세요.
</Aside>
...
| Query  | direction     | 필수                | [쿼리 방향](#query-direction) (OUT 또는 IN) |
| Query  | ranges        | 선택 (기본값: null) | [스캔 범위](#index-ranges)                  |
...
| Query  | filters       | 선택 (기본값: null) | [필터링 조건](#filters)           |
...
Retrieve multiple multi-edges by their IDs. For unique-edge vs multi-edge, see [FAQ](/ko/faq/#what-is-the-difference-between-unique-edge-and-multi-edge).
...
| Header | Authorization | 선택                     | 인증 키 (향후 사용을 위해 예약됨)            |
| Path   | database      | 필수                     | 타겟 데이터베이스 이름                       |
| Path   | table         | 필수                     | 타겟 테이블 이름 (멀티 엣지 테이블이어야 함) |
| Query  | ids           | 필수                     | 엣지 ID 목록 (쉼표로 구분)                   |
| Query  | filters       | 선택 (기본값: null)      | [필터링 조건](#filters)                      |
| Query  | features      | 선택 (기본값: 빈 리스트) | 포함할 기능 목록                             |
...
| Query  | offset        | 선택 (기본값: null)      | [페이지네이션](#pagination) 오프셋            |
| Query  | ranges        | 선택 (기본값: null)      | [스캔 범위](#index-ranges)                    |
| Query  | filters       | 선택 (기본값: null)      | [필터링 조건](#filters)                       |
...
`filters` 파라미터를 사용하면 get/scan 쿼리에서 데이터(엣지)를 필터링할 수 있습니다. [범위](#index-ranges)와 동일한 형식을 사용합니다.

**범위와의 차이점:** [범위](#index-ranges)는 쿼리 범위를 사전에 지정하며 인덱스로 설정된 필드만 사용할 수 있는 반면, `filters`는 쿼리 결과를 필터링하며 인덱스로 설정되지 않은 필드도 사용할 수 있습니다.
