---
title: Build Your Social Media App
description: Build social features using Actionbase core functionality
sidebar:
  order: 1
---

import { Steps, Aside } from '@astrojs/starlight/components';
import {
  Users,
  Heart,
  Home,
  Package,
  Table,
  Search,
  Zap,
  BarChart,
  List,
  Check,
  Rocket,
  UserPlus,
  RefreshCcw,
} from '@lucide/astro';

<Aside type="tip">
  New to Actionbase? Start with [Quick Start](/quick-start/) for a concise introduction.
</Aside>

This guide walks you through building a simple social media application using Actionbase. You will learn how to model and serve activity data for core features such as **follows**, **likes**, and **feeds**.

<video autoplay loop muted playsinline style="width: 100%; border-radius: 8px; margin: 1rem 0;">
  <source
    src="https://github.com/kakao/actionbase/releases/download/examples/hero.webm"
    type="video/webm"
  />
</video>

## What You Will Build

By the end of this guide, you will have implemented:

- **Follows** - Users can follow other users
- **Likes** - Users can like posts
- **Feed** - Display posts from followed users with real-time like counts

These features demonstrate the core pattern behind most social applications.

## Prerequisites

- Docker
- Web browser

## Start the Interactive Guide

This guide has an interactive component that runs locally.

<Steps>

1. **Start Actionbase with Docker**

   ```bash
   docker run -it -p 9300:9300 ghcr.io/kakao/actionbase:standalone
   ```

2. **Start the interactive guide**

   Once the CLI prompt (`actionbase>`) appears:

   ```
   guide start hands-on-social
   ```

3. **Screen layout**

   You will see the screen with three panels:
   - **Left**: Progress sidebar showing guide steps
   - **Center**: Social app UI
   - **Right**: CLI terminal and API logs

   A popup will walk you through each step. If something goes wrong, click <span class="icon-row"><RefreshCcw size={14} /> **Restart**</span> in the top-left corner.

   ![Screen layout](../../../assets/guides/social-media/00-screen-layout.png)

4. **Open in your browser**

   ```
   http://localhost:9300
   ```

   The sections below summarize what you'll see — you can read through without running the guide, or use them as a reference while following along.

</Steps>

## Follow the Guide

From here, continue in the **web browser**. The interactive guide walks you through each step.

<Steps>

1.  **Welcome**

    Welcome to the Actionbase hands-on guide! In this tutorial, you will:

    <ul class="icon-rows">
      <li class="icon-row">
        <Users size={18} /> Build follows
      </li>
      <li class="icon-row">
        <Heart size={18} /> Build likes
      </li>
      <li class="icon-row">
        <Home size={18} /> See your feed
      </li>
    </ul>

    ![Welcome](../../../assets/guides/social-media/01-welcome.png)

2.  **You are @zipdoki**

    You will play as **@zipdoki** throughout this tutorial.

    **Tip:** Press **Enter** to proceed.

    ![Zipdoki intro](../../../assets/guides/social-media/02-zipdoki-intro.png)

3.  **Set Up**

    First, let's load **sample data** so you can focus on building.

    <ul class="icon-rows">
      <li class="icon-row">
        <Package size={18} /> Create database & tables
      </li>
      <li class="icon-row">
        <Users size={18} /> Add sample users & posts
      </li>
    </ul>

    ![Set Up](../../../assets/guides/social-media/03-set-up.png)

4.  **Load Sample Data**

    Click **Run** to create:

    <ul class="icon-rows">
      <li class="icon-row">
        <Package size={18} /> Database with users
      </li>
      <li class="icon-row">
        <Table size={18} /> Posts & likes tables
      </li>
    </ul>

    ```bash
    load preset build-your-social-app
    ```

    ![Load sample data](../../../assets/guides/social-media/04-load-sample-data.png)

5.  **Select Database**

    Switch to the `social` database.

    <ul class="icon-rows">
      <li class="icon-row">
        <Package size={18} /> Use database social
      </li>
    </ul>

    ```bash
    use database social
    ```

    ![Select database](../../../assets/guides/social-media/05-select-database.png)

6.  **Explore the Data**

    In the previous step, we created these tables:

    <ul class="icon-rows">
      <li class="icon-row">
        <Table size={18} /> user_posts — who posted what
      </li>
      <li class="icon-row">
        <Heart size={18} /> user_likes — who liked which post
      </li>
    </ul>

    Browse around before we add new interactions.

    ![Explore the Data](../../../assets/guides/social-media/06-explore-the-data.png)

7.  **Follows**

    Let's build a **follow** feature.

    <ul class="icon-rows">
      <li class="icon-row">
        <Table size={18} /> Create a table
      </li>
      <li class="icon-row">
        <Users size={18} /> Add a relationship
      </li>
      <li class="icon-row">
        <Search size={18} /> Query it
      </li>
    </ul>

    ![Follows](../../../assets/guides/social-media/07-follows.png)

8.  **Create Follows Table**

    Create a `user_follows` table.

    <ul class="icon-rows">
      <li class="icon-row">
        <UserPlus size={18} /> Who follows whom
      </li>
    </ul>

    ![Create Follows Table](../../../assets/guides/social-media/08-create-follows-table.png)

9.  **Follow a User**

        Make **@zipdoki** follow **@j4rami**. This creates a connection between two users.

        <ul class="icon-rows">
          <li class="icon-row">
            <Zap size={18} /> Precomputing count & index
          </li>
        </ul>

        ```bash
        mutate user_follows --type INSERT --source zipdoki --target j4rami ...
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl -X POST "http://localhost:9300/graph/v3/databases/social/tables/user_follows/edges" \
            -H "Content-Type: application/json" \
            -d '{
              "mutations": [{
                "type": "INSERT",
                "edge": {
                  "version": 1737849600000,
                  "source": "zipdoki",
                  "target": "j4rami",
                  "properties": { "createdAt": 1737849600000 }
                }
              }]
            }'
        ```

        </details>

        ![Follow a User](../../../assets/guides/social-media/09-follow-a-user.png)

10. **Check Follow Status**

        Verify the follow exists.

        <ul class="icon-rows">
          <li class="icon-row">
            <Check size={18} /> Query relationship
          </li>
        </ul>

        ```bash
        get user_follows --source zipdoki --target j4rami
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl "http://localhost:9300/graph/v3/databases/social/tables/user_follows/edges/get?source=zipdoki&target=j4rami"
        ```

        </details>

        ![Check Follow Status](../../../assets/guides/social-media/10-check-follow-status.png)

11. **Count Followers**

        Get **@j4rami**'s follower count.

        <ul class="icon-rows">
          <li class="icon-row">
            <Rocket size={18} /> No aggregation
          </li>
        </ul>

        ```bash
        count user_follows --start j4rami --direction IN
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl "http://localhost:9300/graph/v3/databases/social/tables/user_follows/edges/count?start=j4rami&direction=IN"
        ```

        </details>

        ![Count Followers](../../../assets/guides/social-media/11-count-followers.png)

12. **List Followers**

        Get the list of users following **@j4rami**.

        <ul class="icon-rows">
          <li class="icon-row">
            <Rocket size={18} /> Already indexed
          </li>
        </ul>

        ```bash
        scan user_follows --start j4rami --index created_at_desc --direction IN
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl "http://localhost:9300/graph/v3/databases/social/tables/user_follows/edges/scan/created_at_desc?start=j4rami&direction=IN"
        ```

        </details>

        ![List Followers](../../../assets/guides/social-media/12-list-followers.png)

13. **Likes**

    Now let's add **likes**. Same pattern as follows.

    <ul class="icon-rows">
      <li class="icon-row">
        <Heart size={18} /> A user interacts with a post
      </li>
    </ul>

    ![Likes](../../../assets/guides/social-media/13-likes.png)

14. **Like a Post**

        Make **@zipdoki** like **@j4rami**'s post.

        <ul class="icon-rows">
          <li class="icon-row">
            <Zap size={18} /> Precomputing count & index
          </li>
        </ul>

        ```bash
        mutate user_likes --type INSERT --source zipdoki --target 1 ...
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl -X POST "http://localhost:9300/graph/v3/databases/social/tables/user_likes/edges" \
            -H "Content-Type: application/json" \
            -d '{
              "mutations": [{
                "type": "INSERT",
                "edge": {
                  "version": 1737849600000,
                  "source": "zipdoki",
                  "target": 1,
                  "properties": { "createdAt": 1737849600000 }
                }
              }]
            }'
        ```

        </details>

        ![Like a Post](../../../assets/guides/social-media/14-like-a-post.png)

15. **Check Like Status**

        Verify that **@zipdoki**'s like was recorded.

        <ul class="icon-rows">
          <li class="icon-row">
            <Check size={18} /> Query like status
          </li>
        </ul>

        ```bash
        get user_likes --source zipdoki --target 1
        ```

        <details>
        <summary>curl</summary>

        ```bash
        curl "http://localhost:9300/graph/v3/databases/social/tables/user_likes/edges/get?source=zipdoki&target=1"
        ```

        </details>

        ![Check Like Status](../../../assets/guides/social-media/15-check-like-status.png)

16. **And More**

    Just like follows, you can:

    <ul class="icon-rows">
      <li class="icon-row">
        <BarChart size={18} /> Count likes
      </li>
      <li class="icon-row">
        <List size={18} /> List who liked a post
      </li>
    </ul>

    Same pattern, same simplicity.

    ![And More](../../../assets/guides/social-media/16-and-more.png)

17. **Feed**

    Your **feed** now shows:

    <ul class="icon-rows">
      <li class="icon-row">
        <Users size={18} /> Posts from users you follow
      </li>
      <li class="icon-row">
        <Heart size={18} /> Real like counts
      </li>
    </ul>

    This is the core pattern behind most social apps.

    ![Feed](../../../assets/guides/social-media/17-feed.png)

18. **All Done!**

    You just built a **feed** with **follows** and **likes** — all powered by Actionbase.

    Now try it yourself:

    <ul class="icon-rows">
      <li class="icon-row">
        <Search size={18} /> Follow someone
      </li>
      <li class="icon-row">
        <Home size={18} /> Check your feed
      </li>
      <li class="icon-row">
        <Heart size={18} /> Like a post
      </li>
    </ul>

    ![All Done](../../../assets/guides/social-media/18-all-done.png)

19. **Try It Yourself**

    Now it's your turn to explore. The guide is complete, but the app is fully functional:
    - **Follow more users** - Search for users and build your network
    - **Check your feed** - See posts from people you follow
    - **Like posts** - Interact with content in your feed

    ![Try it yourself](../../../assets/guides/social-media/19-try-it-yourself.png)

</Steps>

## Summary

You just built a feed with follows and likes - all powered by Actionbase.

### What You Learned

| Concept            | Description                                                    |
| ------------------ | -------------------------------------------------------------- |
| **Tables**         | Create tables with schemas, indexes, and bidirectional queries |
| **Mutations**      | Insert edges representing user interactions                    |
| **Queries**        | Get individual edges, count totals, and scan with indexes      |
| **Precomputation** | Counts and indexes are computed at write time for fast reads   |

### Next Steps

- [Quick Start](/quick-start/) — Core operations in minutes
- [CLI Reference](/operations/cli/) — Full CLI documentation
- [Core Concepts](/design/concepts/) — How Actionbase works

### Share Your Feedback

Was this guide helpful? Did Actionbase's concepts make sense? We'd love to hear your thoughts — questions, suggestions, or issues are all welcome.

[Open an issue on GitHub](https://github.com/kakao/actionbase/issues)

### Behind the Scenes

Curious why we built this guide? [Read the story](https://github.com/kakao/actionbase/issues/454).
