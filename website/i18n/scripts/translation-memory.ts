...
function loadTm(
  ...
  if (typeof data === "object" && !Array.isArray(data)) {
    ...
    contributors =
      ((d.meta as Record<string, unknown>)?.contributors as string[]) || [];
  } else {
...
function emitTableBlock(
  ...
  for (let idx = 0; idx < tableLines.length; idx++) {
    if (sepIndices.includes(idx)) {
      ...
      if (rowData) {
        const [, cells] = rowData;
        const padded = cells.map((c, j) =>
          j < colWidths.length ? padCell(c, colWidths[j]) : c,
        );
...
function cmdBuild(lang: string): number {
  ...
  for (const docRel of enDocs.sort()) {
    ...
    const enContent = fs.readFileSync(enPath, "utf-8");
    const translated = buildTranslatedDoc(
      enContent,
      tm,
      glossaryPreserve,
      lang,
...
function cmdStatus(
  ...
  for (const docRel of enDocs.sort()) {
    ...
    const hits = segments.filter(
      (s) => tm.has(s.text) && tm.get(s.text)!.target,
    ).length;
...
function printTable(
  ...
  const overall =
    totalSegments > 0
      ? `${Math.round((totalHits / totalSegments) * 100)}%`
      : "N/A";
  console.log(
