...
public class BulkEdgeEncoderTests {
  ...
  static final String edgeJsonString = "{\"active\":true,\"ts\":1,\"src\":123,\"tgt\":\"Coffee10\",\"props\":{\"created_at\":1, \"permission\":\"public\", \"memo\":\"for good morning\"}}";
  ...
  void testFetchIndexedLabelAndEncodeEdges() throws JsonProcessingException {
    ...
    LabelDTO newLabel = label.copy("gift.like_product_v1_20240402_132500",
        "gift.like_product_v1_20240402_132500");
    ...
    List<KeyFieldValue<byte[]>> encodedEdges = BulkEdgeEncoder.bulkEncodeAll(encoder, edge,
        newLabel);
    ...
    encodedEdges.forEach(
        kv -> {
          if (kv.key.length != 0) {
            ...
            long matchCount = expectedCounterKeys.stream().filter(k -> Arrays.equals(k, kv.value))
                .count();
  ...
  void testFetchIndexedLabelAndEncodeEdgesOutboundOnly() throws JsonProcessingException {
    String labelJsonString1 = labelJsonString.replace("\"dirType\":\"BOTH\"",
        "\"dirType\":\"OUT\"");
