...
class ActionbaseObjectMapperTest {
  ...
  void testTypeInferenceJsonConversion() throws Exception {
    // Given
    List<DatabaseDescriptor> databases = Arrays.asList(
    ...
    Page<DatabaseDescriptor> deserializedPage = ActionbaseObjectMapper.fromJson(json,
        new TypeReference<Page<DatabaseDescriptor>>() {
        });
