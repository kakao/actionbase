...
class HBaseDatastoreBindingConfiguration(
    ...
    fun hBaseAdmin(): HBaseAdmin {
        val backend = DefaultStorageBackendFactory.INSTANCE
        val hbaseBackend =
            backend as? HBaseStorageBackend
                ?: throw IllegalStateException("HBaseStorageBackend is required for HBaseAdmin binding.")
        return HBaseAdmin(
            hbaseBackend.connectionMono
                .map { it.admin }
                .cache(),
        )
    }
