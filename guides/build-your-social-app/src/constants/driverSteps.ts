...
export const stepNextEvent = new Map<number, StepEvent>([
  ...
  [8, {target: ["[id='btn-profile-following']", "[id='run-command-btn']"]}],
  [9, {target: ["[id='run-command-btn']"]}],
  [10, {target: ["[id='run-command-btn']"]}],
  [11, {target: ["[id='run-command-btn']"]}],
  [12, {to: '/post/1', target: ["[id='btn-likes']", "[id='run-command-btn']"]}],
  [13, {target: ["[id='run-command-btn']"]}],
  [14, {target: ["[id='run-command-btn']"]}],
  [15, {to: '/'}],
]);
...
stepVerifiers.set(7, async () => {
  ...
  if (!userPosts || userPosts?.active === false) {
    ...
    return false;
  } else if (!userFollows || userFollows?.active === false) {
    return false;
  }
  return true;
});
...
setDelegatingVerifiers([9, 10, 11, 12], 8);
setDelegatingVerifiers([14], 13);
