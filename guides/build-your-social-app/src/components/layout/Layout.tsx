...
const Layout: React.FC<SplitLayoutProps> = ({children}) => {
  ...
  const {currentStep, setCurrentStep, setButtonType, setNavigationUrl, setRefresh} = useDriver();
  const [stableStep, setStableStep] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (latestStep != null && latestStep >= 0) setStableStep(latestStep);
    }, 300);
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    const isStepCompleted = stableStep >= totalSteps;
    const lastIndex = BREADCRUMB_STEPS.length - 1;
    return BREADCRUMB_STEPS.map((step, index) => {
      const isActive = isStepCompleted ? index === lastIndex : index === stableStep;
      const isCompleted = index < stableStep || (isStepCompleted && index < lastIndex);
      return (
        <div key={index} className={`breadcrumb-item ${isActive ? 'active' : ''} ${isCompleted ? 'completed' : ''}`}>
  ...
  return (
    <>
      ...
      <ApiLogProvider>
        ...
        <button className="tutorial-refresh-btn" onClick={handleRefresh}>
